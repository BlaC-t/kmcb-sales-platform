-- Sales Platform BMS menu/permission transfer template
-- Before execution replace every __BMS_TARGET_SCHEMA__ token with the approved BMS schema.
-- This template writes BMS menu/permission metadata only. It never targets the Sales Platform schema on 121.
-- Review IDs, target schema, backup and change window before DEV-to-PROD transfer; do not edit routes.ts.

START TRANSACTION;

INSERT INTO __BMS_TARGET_SCHEMA__.sys_permission
...
FROM __BMS_TARGET_SCHEMA__.sys_permission
